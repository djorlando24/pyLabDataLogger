...
{
  /* Note: The way the device works, if the number of samples
     requested is greater than WMaxPackdtSize/2 (typically 256), then
     the device returns data in chunks of 256.  Therefore the buffer
     must be large enough to hold the extra samples.  For example, if
     nSample = 257, then the size of the data array must be 512 or
     2*256.  The call to usb_bulk_read will fail if you don't read the
     exact amount being sent by the device.
  */
  ...
  if (nbytes < wMaxPacketSize) {
    ...
    ret = libusb_bulk_transfer(udev, LIBUSB_ENDPOINT_IN|6, (unsigned char *) data, nPackets*wMaxPacketSize, &transferred, HS_DELAY);

    // if nbytes is a multiple of wMaxPacketSize the device will send a zero byte packet.
    if (remainder == 0) {
      ret = libusb_bulk_transfer(udev, LIBUSB_ENDPOINT_IN|6, (unsigned char *) value, 2, &transferred, 100);
    }

    if (ret < 0) {
      perror("usbAInScanRead_USB2020: error in libusb_bulk_transfer. (3)");
      return ret;
    }
  }
