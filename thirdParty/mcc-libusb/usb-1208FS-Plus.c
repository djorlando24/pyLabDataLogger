				    ...
				    double frequency, uint8_t channels, uint8_t options)
{
  /*
     ...
     soon as daoota is available from the A/D.  In immediate transfer
...
int usbAInScanRead_USB1208FS_Plus(libusb_device_handle *udev, int nScan, int nChan, uint16_t *data, uint8_t options)
{
  int i;
  int ret = -1;
...
void usbAOutScanStart_USB1208FS_Plus(libusb_device_handle *udev, uint32_t count, double frequency, uint8_t options)
{
  ...
  */
  ...
  struct AOutScan_t {
    uint32_t count;         // The total number of scan to perform (0 for continuous scan)
