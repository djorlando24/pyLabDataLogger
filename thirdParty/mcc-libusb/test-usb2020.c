...
int main (int argc, char **argv)
{
  ...
  while(1) {
	...
	ret = usbAInScanRead_USB2020(device.udev, nSamples, 1, &dataAIn[0]);
	...
	usbAInScanClearFIFO_USB2020(device.udev);
        break;
      case 'C':
	printf("Test for coninuous scan at 1000Hz. System will hang, hit ^C to exit.\n");
   	usbAInScanStop_USB2020(device.udev);
	usbAInScanClearFIFO_USB2020(device.udev);
	...
	usbAInScanStart_USB2020(device.udev, nSamples, 0, frequency, nSamples-1, options);
        i = 0;
        while(1) {
	  ret = usbAInScanRead_USB2020(device.udev, 1000, 1, &dataAIn[0]);
          if (ret != 2000) printf("Error. ret = %d\n.",ret);
	  printf("Scan = %d.\n", i);
          i++;
	}
        break;
